...
*/
    ...
    Apio.Serial.send = function(data, callback) {
        ...
        if(typeof data === "object"){
            var keys = Object.keys(data.properties);
            console.log("dataaaaaaaaaaaaaaaaaa vale:");
            console.log(data);
                ...
                getStateByName(stateName, function(err, state) {
                    ...
                    if (err) {
                        ...
                        Apio.Database.updateProperty(state, function() {
                            ...
                            //console.log(state);
                            Apio.Database.db.collection("Objects").findOne({ objectId : state.objectId }, function(err, r){
                                if(err){
                                    console.log("Error while finding object with ID "+state.objectId);
                                } else {
                                    //console.log("CERCO L'OGGETTO CON ID "+state.objectId);
                                    //console.log("R VALE:");
                                    //console.log(r);
                                    state.address = r.address;
                                    Apio.io.emit("apio_server_update", state);
                                    Apio.io.emit("apio_client_update", state);
